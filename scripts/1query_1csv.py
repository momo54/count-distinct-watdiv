...
#base_dirs = [f"{ROOT}/output/CRAWD", f"{ROOT}/CHAOLEE"]
base_dirs = [f"{ROOT}/output/CRAWD"]


def merge_results(base_dirs):
        ...
        for config in range(1, 10):
            ...
            for filename in os.listdir(path):
                if filename.endswith(".result"):
                    ...
                    print(",".join(parse_results(query_file)))

if __name__ == "__main__":
    ROOT="/Users/molli-p/count-distinct-watdiv"
    #base_dirs = [f"{ROOT}/output/CRAWD", f"{ROOT}/CHAOLEE"]
    base_dirs = [f"{ROOT}/output/CRAWD"]
    merge_results(base_dirs)
